...
Deno.test({
  ...
  async fn(t) {
    for (const {
      id,
      extent: { spatial, temporal, vertical },
    } of colls) {
      await t.step({
        ignore: !vertical,
        ...
        async fn(t) {
          await t.step({
            ...
            fn() {
              vertical?.values?.every((v) => {
                ...
                if (v.includes("/")) {
                  (v.startsWith("R") ? v.substring(1) : v)
                    .split("/")
                    .every((p) => expect(parseFloat(p)).not.toBeNaN());
                }
              });
            },
          });
          ...
          await t.step({
            ignore: vertical?.name === undefined,
            name: "vrs",
            fn() {
      ...
      await t.step({
        ...
        async fn(t) {
          await t.step({
            ...
            async fn(t) {
              await t.step({
                name: "First value",
                fn() {
                  expect(typeof Date.parse(temporal.interval[0][0]!)).toBe(
                    "number"
                  );
                },
              });
              await t.step({
                name: "Second value",
                fn() {
                  expect(typeof Date.parse(temporal.interval[0][1]!)).toBe(
                    "number"
                  );
                },
              });
          ...
          await t.step({
            ignore: temporal.values === undefined,
            name: "values",
            fn() {
      ...
      await t.step({
        ...
        async fn(t) {
          await t.step({
            name: "Bbox: Tests only first array",
            async fn(t) {
              await t.step({
                ignore: spatial.bbox[0].length === 6,
                name: "Four-Item",
                fn() {
                  expect(spatial.bbox[0][0]).toBeLessThanOrEqual(
                    spatial.bbox[0][2]
                  );
                  expect(spatial.bbox[0][1]).toBeLessThanOrEqual(
                    spatial.bbox[0][3]
                  );
                },
              });
              await t.step({
                ...
                fn() {
                  ...
                  expect(
                    spatial.bbox[0][2],
                    "Lower axis height value"
                  ).toBeLessThanOrEqual(spatial.bbox[0][5]);
                },
              });
            },
          });
