...
import type { Collection, Extent, Parameter } from "../types.d.ts";
import config, {
  type ExtentProps,
...
function getInstances(ctx: ExegesisContext) {
  ...
  const format = parseFormat(
    ...
    dataset.data_queries.instances?.output_formats || dataset.output_formats
  );
...
function toExtent(props: ExtentProps): Extent {
  ...
  if (props.z !== null)
    props.z = Array.from(new Set(props.z))
      .filter((p) => p !== null)
      .sort((a, b) => a - b);
  return {
    spatial: {
    ...
    temporal: {
      ...
      interval:
        props.datetime === null
          ? [[null, null]]
          : [[props.datetime[0], props.datetime[props.datetime.length - 1]]],
      values: props.datetime,
    },
    vertical: {
      interval:
        props.z === null
          ? [[null, null]]
          : [[props.z[0].toString(), props.z[props.z?.length - 1].toString()]],
      values:
        props.z === null ? null : props.z?.map((p) => p.toString()) || null,
      vrs: props.vrs,
    },
  };
}

function toDataQueries(ctx: ExegesisContext, dq: DataQueryConfig) {
